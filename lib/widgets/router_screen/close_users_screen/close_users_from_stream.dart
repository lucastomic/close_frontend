...
class CloseUsersFromStream extends StatefulWidget {
  ...
  CloseUsersFromStream(this._closeUsersService,this._qualityReducer, this._messageService, this._duckService,{super.key});
...
class _CloseUsersFromStreamState extends State<CloseUsersFromStream> {
  ...
  Widget _buildWidgetFromSnapshot(_, AsyncSnapshot<List<User>> snapshot) {
    if (_snapshotHasLoaded(snapshot)) {
      return UsersButtonsList(snapshot.data!,widget._qualityReducer, widget._messageService,widget._duckService);
    } else {
